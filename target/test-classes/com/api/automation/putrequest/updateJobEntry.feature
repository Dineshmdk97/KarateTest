...
Scenario: To update the job entry for existing job by calling another feature file
# CREATE [by calling another feature file]
# <Gherkin keyword> <Call> <read(location of feature file)>
# <Call> is another keyword coming from the Karate framework
And def postrequest = call read("../postrequest/data/createJobEntry.feature")
And print "Calling createJobEntry feature => ", postrequest.id
...
# Validation of updated array is equal to 2, after adding an additional array
# For more understanding on Operators -> https://github.com/json-path/JsonPath
And def updatedProjectArray = karate.jsonPath(response, "$[?(@.jobId == "+postrequest.id+")].project")
And print updatedProjectArray
# Result of jsonPath() API is an array so we need to use the index as [0]
And match updatedProjectArray[0] == '#[2]'


Scenario: To update the job entry for existing job by calling another feature file with variables
# CREATE [by calling another feature file with variables
...
Given call read("../postrequest/data/createJobEntryWithVariables.feature") {_url: 'http://localhost:9897', _path: '/normal/webapi/add', _id: '#(id)'}
...
{
    "jobId": "#(id)",
    "jobTitle": "Software Engineering Role 2.0",
    "jobDescription": "To develop andriod application",
    "experience": [
        "Google",
        "Apple",
